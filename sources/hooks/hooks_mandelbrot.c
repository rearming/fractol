...
/*   Updated: 2019/05/30 14:47:01 by sleonard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		zoom_mandelbrot(t_mlx *mlx, int mode)
{
	...
	if (mode == OUT)
	{
		mlx->mp.xside += mlx->mp.xside * 0.05;
		mlx->mp.yside += mlx->mp.yside * 0.05;
		mlx->mp.hor_shift -= (mlx->mp.xside * 0.05)
				/ round(mlx->mp.scale_x);
		mlx->mp.vert_shift -= (mlx->mp.yside * 0.05)
				/ round((mlx->mp.scale_y * 2));
	}
}
...
void		mouse_change_mandelbort(t_point pos, t_mlx *mlx)
{
	pos.x = pos.x >= WIN_WIDTH / 2 ?
			pos.x - (WIN_WIDTH - pos.x) : -WIN_WIDTH / 2 + pos.x;
	pos.y = pos.y >= WIN_HEIGHT / 2 ?
			pos.y - (WIN_HEIGHT - pos.y) : -WIN_HEIGHT / 2 + pos.y;
